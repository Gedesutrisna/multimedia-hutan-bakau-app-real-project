...
@section('container')
    ...
    <div class="content-main p-[32px]">
        <div class="flex gap-3">
            ...
            <p class="text-[#141414] text-[28px] font-Urbanist font-semibold">Edit Video Blog {{ $vlog->title }}</p>
        </div>
        <form action="/dashboard/vlogs/{{ $vlog->slug }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1mt-9 gap-4">
            <div class="">
        ...
        <div class="grid grid-cols-1 mt-9 gap-4">
            <div class="">
                ...
                <textarea name="body" id="body" value="{{ old('body', $vlog->body) }}"
                    type="text"
                    style="resize: none" @error('body') is-invalid @enderror
                >{{ old('body', $vlog->body) }}</textarea>
        ...
        <div class="grid grid-cols-1mt-9 gap-4">
            <div class="">
                ...
                <input name="url" id="url" value="{{ old('url',$vlog->url) }}"
                    type="text"
                    class="mt-2 py-[18px] px-[16px] w-full border border-[#E1E2E6] rounded-[4px] focus:outline-none"
        ...
        <input type="hidden" class="form-control @error('slug') is-invalid @enderror form-control-sm" name="slug" id="slug" value="{{ old('slug', $vlog->slug) }}" required>
        <div class="flex items-center gap-2 mt-[26px]">
            <button type="submit" class="py-[14px] px-4 bg-[#428574] text-white rounded-[8px]">Edit Video Blog</button>
